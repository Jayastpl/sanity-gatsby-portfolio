...
const SearchLangCode= React.forwardRef((props, ref) => {
      ...
      //   }
       ...
       useEffect(() => {
        const LANGUAGE_URL = ((process.env.SANITY_STUDIO_API_DATASET == 'production')? configSetting.productionApiUrl.getlocalUrl : configSetting.StagingApiUrl.getlocalUrl  )  + `${brandId}/${countryId}`;
        setIsFetching(true);
          try{
            fetch(LANGUAGE_URL)
            .then(res => res.json())
            .then(response => {
                console.log("my language code",response);
